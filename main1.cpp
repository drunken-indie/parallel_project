...
void calculateForce(body** bodies, int** processbody, int a, int b);
            ...
            for (i = 0; i < p; i++){
                if (i == 0){
                    if (my_rank == MASTER){
                        ...
                        for (i = 0; i <total_particles; i++){
                            to = i%p;
                            MPI_Send(&i, 1, MPI_INT, to, 0, MPI_COMM_WORLD);
                            //printf("Rank %d: sent %d structure body to %d Rank\n", my_rank, i, to);
                        }
                    }
                    ...
                    if (my_rank >= MASTER){
                        processbody = (int*)malloc((2*(total_particles/p))*sizeof(int));
                        ...
                        //CALCULATEEEE
                        calculateForce(&bodies, &processbody, 0, 2);
                        //printf("a: %d \n", processbody[0].type);
                        if (my_rank == MASTER){
