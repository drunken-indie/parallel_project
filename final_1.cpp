...
void print_status(body **bodies, int total_particles);
...
int main(int argc, char* argv[]){
	...
	if(my_rank == MASTER){
		...
		printf("%d, %d \n", numParticlesLight, numParticleMedium);
		total_particles = numParticlesLight+numParticleMedium+numParticleHeavy;
		bodies = new struct body[total_particles];
	...
	if(my_rank == MASTER){
		print_status(&bodies, total_particles);
...
void print_status(body **bodies, int total_particles){
	...
	for (i = 0; i < total_particles; i++){
		printf("particle %d has type : %d with x position : %f, y position : %f, x velocity : %f, y velocity : %f \n", i, (*bodies)[i].type, (*bodies)[i].position.x, (*bodies)[i].position.y, (*bodies)[i].velocity.x, (*bodies)[i].velocity.y);
	}
}
void saveIt(body **bodies, char *filename, int width, int height, int total_particles){
	int i;
	...
	if (my_rank == MASTER){
		...
		for (i = 0; i < total_particles; i++){
			...
			for (a = tempy; a < tempy+4; a++){
				for (b=tempx; b < tempx+4; b++){
					//printf("a : %d, b : %d \n", a, b);
					//printf("%d \n", (int(tempcolor.x)*255));
